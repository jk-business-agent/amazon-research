...
- When running commands to create files, ask first if it is an "Output File". If the answer is yes, save it to the output folder.
    - All docs, reports, analysis, etc. created by agent tasks and saved to output/ should be maximally organized, clear, readable, and useful. Agent should avoid falling into templated production habits not explicitly requested in instructions or the prompt.
...
- output/ - finished deliverables requested (reports, drafts, analysis completed by the agent)
  - output/index.html - archive index of all daily dashboards; regenerate after writing each new report
