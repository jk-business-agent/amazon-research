...
This workflow runs fully unattended (scheduled, no human in the loop). Before every run, the cloud routine should execute `bash scripts/clear-stale-lock.sh` to remove any stale `.claude/scheduled_tasks.lock` file left by a previous failed or interrupted run — this is a bash script specifically because the unattended cloud sandbox has no PowerShell runtime available. Do not ask clarifying questions
